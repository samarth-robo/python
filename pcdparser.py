...
# original script by Aurel Wildfellner
# Modified by Samarth Brahmbhatt, Georgia Tech to:
# - clean up some bugs
# - parse PointCloud<PointNormal>
...
class PointNormal(Point):
  ...
  c   = 0  # curvature
  def __init__(self):
    super(PointNormal, self).__init__()
  def setField(self, fieldname, values):
    value = values[0]
    if fieldname == 'x':
      self.x = value
      ...
      self.z = value
    elif fieldname == 'normal_x':
      self.n_x = value
    elif fieldname == 'normal_y':
      self.n_y = value
    elif fieldname == 'normal_z':
      self.n_z = value
    elif fieldname == 'curvature':
      self.c = value
...
def parse_file(filename):
  # parser = PCDParser.factory(filename, PointXYZ)
  parser = PCDParser.factory(filename, PointNormal)
  if parser:
    parser.parseFile()
